...
object Server: BaseCommand() {
	...
	fun switch(source: Player, targetServer: String) {
		...
		if (server == null || !source.hasPermission("ion.server.${server.serverInfo.name}")) {
			source.sendMessage(text("Server ${server.serverInfo.name} does not exist, or you can not access it."))
			return
		}
